...
public class trackerEnemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (currentState == TrackerState.AGGRO)
        {
            Aggro();
        }
        else if (currentState == TrackerState.IDLE)
        {
            Idle();
        }

        //Counts down until enemy can charge attack again
        if (idleTimer > 0.0f && !GameManager.GetPaused() && currentState == TrackerState.IDLE)
        {
            idleTimer -= Time.deltaTime;
        }

        //Moves the enemy towards the designated position
    ...
    void setAggro()
    {
        ...
        currentState = TrackerState.AGGRO;
    ...
    void OnDestroy()
    {
        System.Type enemyType = typeof(trackerEnemy);
